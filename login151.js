class Login {
    ...
    static verficaLogin = () => {
        ...
        const endpoind = `${this.config.endpoind}/?matricula=${mat}&senha=${pas}`
        fetch(endpoind)
            .then(res => res.json())
            .then(res => {
                if (res) {
                    sessionStorage.setItem("logando","true")
                    sessionStorage.setItem("matlogando",mat)
                    sessionStorage.setItem("nomelogando",res.nome)
                    sessionStorage.setItem("acessologando",res.acesso)
                    this.callback_ok();
                    this.fechar();
                } else {
                    sessionStorage.setItem("logando", "false")
